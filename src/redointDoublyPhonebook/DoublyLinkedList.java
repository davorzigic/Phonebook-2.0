...
public class DoublyLinkedList {
	...
	public Contact cleanAdd(String s1, String s2, String s3) {
		...
		if (head == null) {
			...
			if (newContact.getLastName().charAt(0) < head.getData()) {
				...
				// searching for the right place for the section and inserting
				// contact
			} else {
	...
	public void deleteContact(Contact name) {

		letterSection helper = head;
		Contact temp = helper.getDown();

		if (helper.getDown() == name) {
			helper.down = temp.down;
			temp.down = null;
		} else {
			while (temp.down != name) {
				temp = temp.down;
			}
			Contact delete = temp.down;
			temp.down = delete.down;
			delete.down = null;
		}
	}
}
